...
exports.createLesson = async (req, res, next) => {
    try {
        const { id: accountId } = req.user; // Lấy ID Teacher đang đăng nhập
        ...
        // 1. XÁC THỰC GIÁO VIÊN VÀ KHÓA HỌC
...
// 2. PUT /api/v1/lessons/:id (Cập nhật thông tin bài học)
exports.updateLesson = async (req, res, next) => {
    try {
        ...
        // Kiểm tra quyền sở hữu nếu là giáo viên
        if (role === 'TEACHER') {
            const teacher = await Teacher.findOne(
                { 
                    where: { idACCOUNT: accountId },
...
// 3. DELETE /api/v1/lessons/:id (Xóa bài học)
...
// 4. GET /api/v1/lessons/:id (Lấy thông tin chi tiết bài học)
...
// 5. GET /api/v1/courses/:courseId/lessons (Lấy danh sách bài học theo khóa học)
